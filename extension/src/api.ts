...
export class ClipSendApi {
    private getServerUrl(): string {
        const config = vscode.workspace.getConfiguration('clipsend');
        return config.get<string>('serverUrl') || 'http://localhost:8081';
    }

    async pairRequest(senderId: string, receiverId: string): Promise<any> {
    ...
    async pairReject(id: string): Promise<void> {
    ...
    async sendText(senderId: string, receiverId: string, text: string): Promise<void> {
    ...
    async sendFile(senderId: string, receiverId: string, filePath: string, fileName: string): Promise<void> {
        const fs = require('fs');
    ...
    async dataList(receiverId: string): Promise<any[]> {
        const res = await fetch(`${this.getServerUrl()}/data/list?receiver_id=${receiverId}`);
        if (!res.ok) return [];
        return res.json();
    }

    async dataAck(id: string): Promise<void> {
